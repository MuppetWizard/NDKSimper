//
// Created by ASUS on 2022/4/25.
//

#ifndef NDKSIMPLE_SAFE_QUEUE_H
#define NDKSIMPLE_SAFE_QUEUE_H

#include <queue>
#include <pthread.h>
#include <rtmp.h>

using namespace std;

template<typename T>

class SafeQueue{
    // java 的回调 === C 的函数指针
    typedef void (*ReleaseCallback)(T *);

    typedef void (*RTMPReleaseCallback)(T &);

    //删除视频的回调
    typedef void (*DeleteFrameCallback)(queue<T> &);

private:
    std::queue<T> q;
    pthread_mutex_t mutex;
    pthread_cond_t cond;
    int flag; //标记队列释放工作(true=工作状态，false=非工作状态)

    ReleaseCallback releaseCallback;
    RTMPReleaseCallback rtmpReleaseCallback;
    DeleteFrameCallback deleteFrameCallback;

public:
    SafeQueue(){
        pthread_mutex_init(&mutex,0);
        pthread_cond_init(&cond,0);
    }

    ~SafeQueue(){
        pthread_mutex_destroy(&mutex);
        pthread_cond_destroy(&cond);
        flag = false;
    }

    /*
     * 入队
     */
    void push(T value){
        pthread_mutex_lock(&mutex); //为了线程安全，上锁
        if (flag){
            q.push(value);
            pthread_cond_signal(&cond);
        } else {
            //释放操作
            //不知道value是什么类型，交给用户处理
            if (releaseCallback) {
                releaseCallback(&value);
            }
            if (rtmpReleaseCallback) {
                rtmpReleaseCallback(&value);
            }
        }
        pthread_mutex_unlock(&mutex); //为了其他线程可进入，解锁
    }

    /*
     * 出队
     */
    int pop(T &t){
        int result = 0;
        pthread_mutex_lock(&mutex); //线程上锁
        while (flag && q.empty()) {
            //如果工作状态，并且队列中没有数据，就等待---排队
            pthread_cond_wait(&cond,&mutex);
        }
        if (!q.empty()) {
            t = q.front();
            q.pop();
            result = 1;
        }

        pthread_mutex_unlock(&mutex); //解锁

        return result;
    }

    void setFlag(int flag){
        pthread_mutex_lock(&mutex); //上锁
        this->flag = flag;

        pthread_cond_signal(&cond); //通知

        pthread_mutex_unlock(&mutex); //解锁
    }

    int isEmpty(){
        return q.empty();
    }

    int queueSize(){
        return q.size();
    }

    void clearQueue(){
        pthread_mutex_lock(&mutex); //上锁


        unsigned int size = q.size();
        for (int i = 0; i < size; ++i) {
            // 循环 一个个释放
            T value = q.front();
            if (releaseCallback) {
                releaseCallback(&value);
            }
            if (rtmpReleaseCallback) {
                rtmpReleaseCallback(&value);
            }
            q.pop();
        }

        pthread_mutex_unlock(&mutex); //解锁
    }

    void setReleaseCallback(ReleaseCallback callback) {
        releaseCallback = callback;
    }
    void setRtmpReleaseCallback(RTMPReleaseCallback callback) {
        rtmpReleaseCallback = callback;
    }

    //设置删除视频帧的回调
    void setDeleteVideoFrameCallback(DeleteFrameCallback delFrameCallback) {
        deleteFrameCallback = delFrameCallback;
    }

    /*
     * 丢弃视频帧
     */
    void deleteVideoFrame(){
        // lock
        pthread_mutex_lock(&mutex);

        deleteFrameCallback(q);

        //unlock
        pthread_mutex_unlock(&mutex);
    }
};

#endif //NDKSIMPLE_SAFE_QUEUE_H
